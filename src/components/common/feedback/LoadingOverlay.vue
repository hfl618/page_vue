...
<template>
  <Transition name="fade">
    <div v-if="uiStore.loading.show" class="fixed inset-0 z-[200000] flex items-center justify-center bg-zinc-900/10 backdrop-blur-xl" translate="no">
      ...
      <!-- 1:1 还原的外层容器 -->
      <div class="border border-zinc-900 shadow-[4px_4px_0px_rgba(0,0,0,0.15)] px-16 py-12 flex flex-col items-center gap-10" 
           style="background-color: rgba(255, 255, 255, 0.4); backdrop-filter: blur(32px); -webkit-backdrop-filter: blur(32px);">
        
        <!-- 核心 Loader 单元 -->
        <div class="w-16 h-16 border-2 border-zinc-900/10 flex items-center justify-center relative">
          <div class="w-6 h-6 bg-zinc-900 heflos-loader-square"></div>
        ...
        <div class="flex flex-col items-center gap-3">
          <div class="flex flex-col items-center text-center">
            ...
            <span class="text-[12px] font-black text-zinc-900 uppercase tracking-tight mt-1">{{ uiStore.loading.subtitle || 'ACCESSING ARCHIVE' }}</span>
          </div>
