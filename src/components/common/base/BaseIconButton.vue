...
<template>
  <button 
    ...
    :class="[
      // 基础样式
      !disabled && !loading ? 'active:translate-x-0.5 active:translate-y-0.5 active:shadow-none cursor-pointer' : 'opacity-50 cursor-not-allowed',
      // 颜色状态
  ...
  >
    <template v-if="loading">
      <div class="w-3 h-3 border-2 border-zinc-200 border-t-zinc-900 animate-spin rounded-full"></div>
    </template>
    <template v-else>
      <slot></slot>
    </template>
